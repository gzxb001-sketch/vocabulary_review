          ...
          <div className="stack">
            {word.sources.map((source) => (
              <div key={source.id} className="list-card">
                <strong>来源：{source.sourceType}</strong>
                <span className="muted">{source.sourceNote || "无备注"}</span>
                {source.sourceContext ? (
                  <span className="muted">{source.sourceContext}</span>
                ) : null}
                <span className="muted">
                  记录时间：{new Date(source.createdAt).toLocaleString()}
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="stack">
            {word.reviews.map((review) => (
              <div key={review.id} className="list-card">
                <strong>结果：{review.reviewResult}</strong>
                <span className="muted">
                  时间：{new Date(review.reviewedAt).toLocaleString()}
                </span>
                <span className="muted">
                  间隔变化：{review.intervalBefore ?? 0} -&gt; {review.intervalAfter ?? 0} 天
                </span>
              </div>
            ))}
          </div>
